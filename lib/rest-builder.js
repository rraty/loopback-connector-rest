...
RequestBuilder.prototype.buffer = function (val) {
  this.template.template.buffer = false === val ? false : true;
  return this;
};
...
RequestBuilder.prototype.operation = function (parameterNames) {
  ...
  return function () {
    ...
    if (
      arguments.length >= 2 &&
      "object" === typeof arguments[arguments.length - 2]
    ) {
      optionsArgument = arguments[arguments.length - 2];
      console.log({optionsArgument})
      parameters["callContext"] = optionsArgument;
    }
    console.log({ params, arguments, optionsArgument });
    ...
    console.log({ parameters, optionsArgument, cb });
    return self.invoke(parameters, optionsArgument, cb);
  };
};
 ...
 */
RequestBuilder.prototype.invoke = function (parameters, options, cb) {
  ...
  if (cb) {
    callback = function (err, response, body) {
      if (err) {
        ...
        if (debug.enabled) {
          debug(
            "Error Response (status code: %d): %j",
            response.statusCode,
            body
          );
        }
  ...
  const r = this._request(req, options, callback);
  return cb.promise;
};
 ...
 * @param {function} cb The callback function
 */
RequestBuilder.prototype._request = function (uri, req, options, cb) {
  ...
  var self = this;
  var context = { request: self.request, req: req, options: options };

  function work(context, done) {
    self.request(req, function (err, res, body) {
      context.res = res;
      done(err, res, body);
    });
  }
