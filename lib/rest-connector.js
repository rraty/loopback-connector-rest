...
RestConnector.prototype.create = function create(model, data, options, callback) {
  this.getResource(model).create(data, options, function(err, body, response) {
    ...
    if (response.statusCode === 200 || response.statusCode === 201) {
      if (callback)  callback(null, body ? body.id : '', '' , body);
    } else {
...
RestConnector.prototype.find = function find(model, id, options, callback) {
  console.log({model, id, options, callback})
 ...
 * @param {function} [callback] The callback function
 */
RestConnector.prototype.all = function all(model, filter, options, callback, ...rest) {
  console.log({all: true, model, filter, options, callback, rest})
  ...
  if (filter && filter.where && (!filter.order) &&
    ...
    if (keys.length === 1 && keys[0] === 'id') {
      // Map findById
      return this.find(model, filter.where.id, options, function(err, result) {
        if (!err) {
          if (!result) {
            result = [];
          } else {
